...
def weighted_return(change_pred: NDArray, change_test: NDArray) -> NDArray:
    ...
    ----------
    change_pred : NDArray
        Predicted price change in fraction. Shape (M, N).
    change_test : NDArray
        True price change in fraction. Shape (M, N).

    Returns
    -------
    NDArray
        The return rate. with shape (M,).
    """
...
def compute_trend_accuracy(change_pred: NDArray, change_test: NDArray) -> float:
    # trend accuracy
...
def compute_return_rate(change_pred: NDArray, change_test: NDArray, mode: str) \
    ...
    -------
    tuple[NDArray, NDArray]:
        trade_returns: The return rate based on the predicted price change for top 1, 3, and all stocks. Shape (M, 3).
    ...
    for i in [1, 3, M]:
        ...
        trade_return = weighted_return(sorted_change_pred, trade_sign*sorted_change_test)
...
def evaluate_stock_trend_prediction(xlast: NDArray, ypred: NDArray, ytest: NDArray):
    """Evaluate the stock trend prediction accuracy and return rate.

    Parameters
    ----------
    xlast : NDArray
        Last price of the stock. Shape (M, N).
    ypred : NDArray
        Predicted price of the stock. Shape (M, N).
    ytest : NDArray
        True price of the stock. Shape (M, N).
    ...
    # trend accuracy
    trend_accuracy, null_accuracy_rise, null_accuracy_drop = compute_trend_accuracy(change_pred, change_test)
    ...
    # Sell return rate
    sell_returns = compute_return_rate(change_pred, change_test, mode='sell')
    perfect_sell_returns = compute_return_rate(change_test, change_test, mode='sell')
